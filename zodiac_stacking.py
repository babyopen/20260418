...
try:
    ...
    print("CatBoost not available, will skip CatBoost in ensemble")
...
class WeightedEnsemble:
    ...
    def predict_proba(self, X):
        ...
        if CATBOOST_AVAILABLE and self.cb_model is not None:
            ...
            ensemble_proba = 0.40 * xgb_proba + 0.30 * rf_proba + 0.20 * lr_proba + 0.10 * cb_proba
        else:
            ensemble_proba = 0.50 * xgb_proba + 0.35 * rf_proba + 0.15 * lr_proba
        
        return ensemble_proba

def main():
    print("=" * 80)
    print("生肖预测2.0 - CatBoost + Weighted Ensemble集成版")
    print("=" * 80)
    ...
    if CATBOOST_AVAILABLE:
        print("    - XGBoost (权重 40%)")
        print("    - RandomForest (权重 30%)")
        print("    - LogisticRegression (权重 20%)")
        print("    - CatBoost (权重 10%)")
    else:
        print("    - XGBoost (权重 50%)")
        print("    - RandomForest (权重 35%)")
        print("    - LogisticRegression (权重 15%)")
    print("=" * 80)
