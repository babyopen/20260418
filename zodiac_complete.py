...
def build_features_core(history, target_idx):
    features = {}
    ...
    recent_20 = history.tail(min(20, len(history)))
    counts_20 = []
    for z in range(1, 13):
        cnt = len(recent_20[recent_20['zodiac'] == z])
        counts_20.append((-cnt, z))
    
    counts_20.sort()
    rank_map = {}
    for rank, (_, z) in enumerate(counts_20, 1):
        rank_map[z] = rank
    
    for z in range(1, 13):
        features[f'z{z}_hot_rank'] = rank_map[z]
        features[f'z{z}_is_hot3'] = 1 if rank_map[z] <= 3 else 0
        features[f'z{z}_is_cold3'] = 1 if rank_map[z] >= 10 else 0
    
    return features

def build_enhanced_features(df, target_idx):
    history = df.iloc[:target_idx].copy()
    current_zodiac = df.iloc[target_idx]['zodiac']
    features = build_features_core(history, target_idx)
    return features, current_zodiac

def build_enhanced_features_for_next(df):
    history = df.copy()
    target_idx = len(df)
    features = build_features_core(history, target_idx)
    return features
...
class EnsembleModel:
    ...
    def predict_proba(self, X):
        ...
        ensemble_proba = (0.5 * xgb_proba + 0.35 * rf_proba + 0.15 * lr_proba)
...
def main():
    ...
    for idx in range(1, len(df)):
        features, label = build_enhanced_features(df, idx)
    ...
    print("\n正在构建第 2026108 期特征...")
    features_next = build_enhanced_features_for_next(df)
